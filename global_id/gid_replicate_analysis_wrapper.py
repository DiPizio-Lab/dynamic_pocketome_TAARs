...
pocket files are parsed once and reshaped once, and every grid point re-clusters
that same frame.

REQUIRES the meta_analysis_class patch that splits pocket_point_dataframe() out of
_within_pdb_clustering() and adds the point_df / run_within_pdb_clustering
arguments. Without it the sweep will re-run the within-PDB HDBSCAN on every grid
point, which it does not need.
...
global_pockets_IoU_voxel.csv, and only the sweep needs the pocket directories.
"""
...
ROOT = OUTPUT_DIR   # global_ID_{state}_only run directories and this module's output

STATE_RESULT_DIR = os.path.join(ROOT, "global_ID_{state}_only".format(state=STATE))
...
POCKET_DIR_TEMPLATE = os.path.join(INPUT_DIR, "{state}_structures", "results", "{prj}", "{rep}",
                                   "pockets_dens")
...
def load_pocket_comparison_table():
    ...
    pocket_comparison_df = pd.read_csv(POCKET_COMPARISON_TABLE)
...
def global_id_composition(pocket_comparison_df):
    ...
    is_binding_site_fraction - share of members flagged orthosteric (needs the
                               re-run with the fixed filter_ortho_site merge)
    """
    composition_rows = []
...
# threshold sweep
def parse_pockets_once(pocket_dirs, saving_loc):
    """
    Everything upstream of the clustering, run a single time. Same call sequence as
    the _run() body of global_id_wrapper_paper.py, stopping before the global ID
    assignment. Returns (meta_obj, summary_df) so the sweep can re-cluster the same
    parsed pockets over and over.
    """
    os.makedirs(saving_loc, exist_ok=True)
    meta_obj = meta.MetaAnalysis(pocket_dirs=pocket_dirs, saving_loc=saving_loc,
                                 isovalue=ISOVALUE, verbose=VERBOSE, dbscan=DBSCAN,
                                 min_atoms=MIN_ATOMS)
    ...
    point_df = meta_obj.pocket_point_dataframe(summary_df)
    return meta_obj, summary_df, point_df
...
def threshold_sweep(pocket_comparison_df):
    ...
    """
    experiments = list_experiments(pocket_comparison_df)
    pocket_dirs = [pocket_dir_for(project, replicate) for project, replicate in experiments]
    meta_obj, summary_df, point_df = parse_pockets_once(pocket_dirs, THRESHOLD_SWEEP_SCRATCH)
    ...
    for iou_thresh, dilation_radius in grid_points:
        voxelized_df = meta_obj.voxel_intersection_over_union_global_id(
            summary_df=summary_df, voxel_size=VOXEL_SIZE, iou_thresh=iou_thresh,
            dilation_radius=dilation_radius, min_cluster_size=MIN_CLUSTER_SIZE,
            point_df=point_df, run_within_pdb_clustering=False)
