...
Answers "did the run actually finish everything, and where exactly did it stop" without
having to grep multi-hundred-MB SLURM logs: every stage of the pipeline (see STAGES below)
writes one cell of logs/pipeline_summary.csv as it starts (RUNNING) and again as it ends
(OK, or ERROR: <message>). A cell stuck on RUNNING after a job has ended means that process
was killed (OOM, walltime, node failure, ...) mid-stage -- the SLURM logs alone can't tell
you that, since a killed job leaves no traceback.
...
Multiple SLURM shards (see analysis_pipeline.py's --num-shards) write this file concurrently
from different nodes, each touching disjoint experiment rows -- but every write still
read-modifies-writes the whole CSV, so concurrent writers need mutual exclusion or they'll
clobber each other's rows. Uses a lockfile (O_CREAT|O_EXCL, portable across the network/CIFS
mounts this pipeline runs on -- see pocket_analysis.py's _copy_executable docstring for why
flock-style locking is avoided here) rather than flock, which is unreliable on some of those
mounts.
"""
import contextlib
import csv
...
def update_checkpoint(state, pdb_id, replicate, stage, status, message='', timestamp=None):
    ...
    concurrently from several processes (see module docstring).
