...
def load(run_dir=None, table=None):
    ...
    if "gene" not in pock_df.columns or pock_df["gene"].isna().any():
        pock_df["gene"] = pock_df["prj"].str.replace(r"^(apo|holo)", "", regex=True).map(gene_selections.gene_dict())
...
def main():
    ...
    pock_df = load(run_dir=os.path.join(META_ANALYSIS_ROOT, "global_ID_combined"))
    out_dir = os.path.join(META_ANALYSIS_ROOT, "global_ID_combined", "global_ID_heatmap")
